...
const MAX_PLAYERS = 40; // --- NEW: Set your max player limit here! ---
...
io.on('connection', (socket) => {
    ...
    socket.on('joinGame', (username) => {
        // --- NEW: Check if the server is already full! ---
        if (Object.keys(players).length >= MAX_PLAYERS) {
            socket.emit('joinError', 'Server is full! (40/40 players). Please try again later.');
            return; // Stop them from joining
        }
